...
public class TspFrame extends JFrame {
  ...
  private void onSolve() throws MqttException, InterruptedException {
    if (cities == null || cities.size() < 2) {
      log.append("\nLoad a file first.\n");
      return;
    }
    tour = NearestNeighborSolver.solve(cities, 0);
    DispatchSolver disSolver = new DispatchSolver(cities);

    double len = NearestNeighborSolver.length(cities, tour);
    List<Integer> Tours = disSolver.dispatch();
    mapPanel.setTour(Tours);
